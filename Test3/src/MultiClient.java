...
//_________________________________이벤트___________________________________        
    ...
    public void actionPerformed(ActionEvent e) {
		  ...
		  if (obj == b2) { 
			  send(sendmesg);
				/*
				 * if(sendmesg == null || sendmesg.length()==0) {
				 * JOptionPane.showMessageDialog(jframe,"글을쓰세요", "경고",
				 * JOptionPane.WARNING_MESSAGE); } else { try {
				 * oos.writeObject(nickname+"#"+sendmesg); } catch (IOException ee) { //try
				 * {oos.writeObject(nickname+"#"+sendmesg); } catch (IOException ee) {
				 * ee.printStackTrace(); } jtf.setText(""); }
				 */ 
		} else if (obj == jbtn) { 
			try {
    ...
    private void send(String message) {
    	if (message == null || message.length() == 0) {
    		JOptionPane.showMessageDialog(jframe,"글을쓰세요", "경고", JOptionPane.WARNING_MESSAGE); 
			 ...
			 //try {oos.writeObject(nickname+"#"+sendmesg); } catch (IOException ee) { 
				ee.printStackTrace();
			} 
			 jtf.setText(""); 
		} 
